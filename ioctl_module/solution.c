...
static struct cdev *my_cdev; // для определения операций над символьным устройством
...
static int mychrdev_open(struct inode *inode, struct file *file)
{
  ...
  char *kbuf = kmalloc(KBUF_SIZE, GFP_KERNEL); // выделяем для каждого приложения/открытия свой буфер
...
static ssize_t mychrdev_read(struct file *file, char __user *buff, size_t length, loff_t *ppos)
{
  char *device_buffer = file->private_data;

  if (device_buffer[100000] == '1') {
    printk(KERN_INFO "kernel_mooc read buffer in if =  %c", device_buffer[0]);
  ...
  if (*ppos >= strlen(device_buffer)) {
    printk(KERN_INFO "kernel_mooc charDev : Reached the end of the device\n");
    *ppos = 0;
    return 0;
  }
  ...
  printk(KERN_INFO "kernel_mooc ---------------------------read------------------------------");
  printk(KERN_INFO "kernel_mooc read sess = %c: buffer = %s, strlen = %d, bytes_to_read = %d *ppos = %d", device_buffer[0], device_buffer, strlen(device_buffer), bytes_to_read, *ppos);
  printk(KERN_INFO "kernel_mooc buffer + *ppos = %s", device_buffer + *ppos);
  printk(KERN_INFO "kernel_mooc copy to US: *%s* strlen = %d", device_buffer + *ppos, strlen(device_buffer));

  bytes_read = bytes_to_read - copy_to_user(buff, device_buffer + *ppos, strlen(device_buffer + *ppos));

  if (bytes_read)
    bytes_read = strlen(device_buffer + *ppos);

  *ppos += bytes_read;

  printk(KERN_INFO "kernel_mooc bytes_read = %d", bytes_read);
  printk(KERN_INFO "kernel_mooc -------------------------------------------------------------");

  return bytes_read;
}


static ssize_t mychrdev_write(struct file *fp, const char *buff, size_t length, loff_t *ppos)
{
  ...
  printk(KERN_INFO "kernel_mooc buff = *%s* strlen = %d len = %d *ppos = %d dev_buf = *%s* bytes_to_write = %d", buff, strlen(buff), length, *ppos, device_buffer, bytes_to_write);
...
static loff_t mychrdev_lseek(struct file *file, loff_t offset, int orig)
{
  ...
  printk(KERN_INFO "kernel_mooc offset = %ld strlen = %d testpos = %ld", offset, strlen(device_buffer), (long)testpos);
...
static int __init init_chrdev(void)
{
  ...
  register_chrdev_region(first, count, MYDEV_NAME);  // регистрируем регион (множество идентификаторов) для возможных экземпляров устройства/модуля

  my_cdev = cdev_alloc(); // выделение памяти под структуру, содержащую все атрибуты устройства
...
static void __exit cleanup_chrdev(void)
{
  ...
  if (my_cdev) // проверка того, что создавали устройство
    cdev_del(my_cdev);
  unregister_chrdev_region(first, count); // освобождаем номер, который зарезирвировали для устройства
}
